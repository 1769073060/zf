...
@Api(tags = "app写字楼我的关注")
@RestController
@RequestMapping("app/appOfficeBuildingPersonalCentre")
public class AppObPersonalCentreController {

    @Resource
    private IObInfoDetailService iObInfoDetailService;

    @PostMapping("getAppObPersonalCentreListPage")
    ...
    public Result getAppObPersonalCentreListPage(Long pageNum, Long pageSize, Long userId,Double userLongitude, Double userLatitude) {
        Result result = null;
        if (isEmpty(userId)) {
        ...
        try {
            String resultData = iObInfoDetailService.getAppObPersonalCentreListPage(pageNum,pageSize,userId,userLongitude,userLatitude);
